...
caps.on('connection', (socket2) => {
  ...
  socket2.on('pickup', (payload) => eventLogger('pickup', payload)); //listener hears pickup calls eventLogger
  socket2.on('in-transit', (payload) => eventLogger('in-transit', payload));
  socket2.on('delivered', (payload) => eventLogger('delivered', payload));
